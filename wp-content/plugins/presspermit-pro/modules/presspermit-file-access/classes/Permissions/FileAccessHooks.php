    ...
    */
    ...
    function act_attachment_access()
    {
        if (PWP::isAttachment()) {
            ...
            if (defined('PRESSPERMIT_TEASER_VERSION') && (!presspermit_is_REQUEST('pp_rewrite') || presspermit_empty_REQUEST('attachment'))) {
    ...
    function act_parseQueryForDirectAccess(&$query)
    {
        ...
        if (empty($query->query_vars['attachment']) || !presspermit_SERVER_var('QUERY_STRING') || (false === strpos(sanitize_text_field(presspermit_SERVER_var('QUERY_STRING', false)), 'pp_rewrite'))) {
            return;
        }
