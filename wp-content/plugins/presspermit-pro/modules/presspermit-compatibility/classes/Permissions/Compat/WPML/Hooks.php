...
class Hooks
{
    ...
    function __construct()
    {
        ...
        $uri = esc_url_raw(presspermit_SERVER_var('REQUEST_URI'));
    ...
    function actClearTranslatorId($post_id, $post_obj)
    {
        ...
        if (empty($iclTranslationManagement) || presspermit_empty_REQUEST('icl_trid') || presspermit_empty_REQUEST('icl_post_language')) {
            return;
        }

        if (!$job_id = $iclTranslationManagement->get_translation_job_id(
            presspermit_REQUEST_int('icl_trid'), 
            pp_permissions_sanitize_entry(presspermit_REQUEST_key('icl_post_language', false)))
        ) {
            return;
        }
