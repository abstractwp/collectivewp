...
class CMSTreeView 
{
    function __construct() {
        ...
        if (presspermit_is_REQUEST('page', 'cms-tpv-page-page')) {
    ...
    function add_page()
    {
        if (!$page_id = presspermit_REQUEST_int('pageID')) {
            return;
        }
        ...
        $parent_id = (presspermit_is_REQUEST('type', 'inside')) ? $ref_post->ID : $ref_post->post_parent;

        $_post_type = presspermit_REQUEST_key('post_type');
    ...
    function add_pages()
    {
        if (!$ref_post_id = presspermit_empty_REQUEST('ref_post_id')) {
            return;
        }
        ...
        $parent_id = (presspermit_is_REQUEST('inside', 'cms_tpv_add_type')) ? $ref_post->ID : $ref_post->post_parent;
    ...
    function move_page()
    {
        if (!$ref_node_id = presspermit_REQUEST_int('ref_node_id')) {
            return;
        }
        ...
        if (isset($args['xsuppress_filters']) && isset($args['post_type']) && ('post' == $args['post_type'])) {
            ...
            $ok_ids = $wpdb->get_col(
                apply_filters(
                    ...
                    "SELECT ID FROM $wpdb->posts WHERE post_type = 'post' AND ID IN ('$id_csv')"
                )
            );
