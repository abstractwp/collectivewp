...
class PostFilters
{
    ...
    function moderate_cap($wp_sitecaps, $orig_reqd_caps, $args)
    {
        ...
        if (presspermit_empty_POST() && !did_action('bbp_template_redirect'))
            return $wp_sitecaps;

        if (!$forum_id = bbp_get_forum_id()) {
            if ($topic_id = presspermit_REQUEST_int('topic_id')) {
                $forum_id = bbp_get_topic_forum_id($topic_id);
                
            } elseif ($reply_id = presspermit_REQUEST_int('reply_id')) {
