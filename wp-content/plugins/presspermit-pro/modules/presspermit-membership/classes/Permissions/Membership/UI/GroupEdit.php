...
class GroupEdit
{
    ...
    function actAgentsSelectionPreDateFields( $id_suffix ) {
        ...
        ?>
            ...
            <td><input type="text" id="pp_member_start" name="pp_member_start" size="9" placeholder="<?php esc_attr_e('date or #days', 'presspermit');?>" title="<?php echo esc_attr('Select membership start date or enter number of days to delay. Blank or zero value means immediate membership.', 'presspermit');?>" /></td>
            </tr>
            
            <tr>
            <td style="vertical-align: middle;"><?php esc_html_e('To:', 'presspermit');?> </td>
            <td><input type="text" id="pp_member_end" name="pp_member_end" size="9" placeholder="<?php esc_attr_e('date or #days', 'presspermit');?>" title="<?php echo esc_attr('Select membership end date or enter duration in days. Blank or zero value means perpetual membership.', 'presspermit');?>" /></td>
            </tr>
            </table>
            </td>
        <?php 
    ...
    function fltAgentsSelectionLabel($suppress, $id_suffix, $args) {
        if ( 'member' == $id_suffix ) {
            ...
            printf( 
                esc_html__( 'Current Selections: %1$s refresh %2$s' ), 
                '<span style="float:right"><small><a href="' . esc_url($url) . '" title="' . esc_attr__('refresh member status', 'presspermit') . '">', 
                '</a></small></span>' 
            );
