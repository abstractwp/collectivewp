    ...
    class PP_Slack extends PP_Module
    {
        ...
        public function settings_slack_channel($args = [])
        {
            ...
            echo '<div class="info">' . esc_html__(
                    'Default Slack channel in which notifications will be sent to. For example #general, @username, or user@email.com. Users can override this on their profile.',
                    'publishpress-pro'
                ) . '</div>';
            echo '</label>';
        }
        ...
        public function send_slack_message($text, $channel, $actions = null)
        {
            ...
            if (empty($channelID)) {
                $error = new WP_Error();
                $error->add('slack_channel_not_found', esc_html__('Slack channel not found.', 'publishpress-pro'));
